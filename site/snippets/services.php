  ...
  <div class="container">
    <div class="text-center mx-auto mb-5 wow fadeInUp" data-wow-delay="0.1s" style="max-width: 600px;">
      <h6 class="text-primary">
        <?php if ($page->subtitleServices()->isEmpty()) : ?>
          <?= $site->find('snippets')->find('services')->subtitle() ?>
        <?php else : ?>
          <?= $page->subtitleServices() ?>
        <?php endif ?>
      </h6>
      <h1 class="mb-4">
        <?php if ($page->titleServices()->isEmpty()) : ?>
          <?= $site->find('snippets')->find('services')->titleText() ?>
        <?php else : ?>
          <?= $page->titleServices() ?>
        <?php endif ?>
      </h1>
    ...
    <div class="row g-4">
      ...
      <?php else : ?>
        <?php foreach ($page->services()->toStructure() as $service) : ?>
          <div class="col-md-6 col-lg-4 wow fadeInUp" data-wow-delay="<?= $service->delay() ?>">
            <div class="service-item rounded overflow-hidden">
              <?php if ($image = $service->images()->toFile()) : ?>
